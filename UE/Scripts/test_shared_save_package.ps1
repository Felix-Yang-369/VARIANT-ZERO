...
$taskExe=Join-Path $taskRoot 'BuildOutput-SharedSave\Windows\VariantZeroUE\Binaries\Win64\VariantZeroUE.exe'
$taskResults=@()
foreach($taskTest in @(@('VZChuyaStoryTest','VZ_CHUYA_STORY: PASS'),@('VZTravelTest','VZ_TRAVEL: PASS'),@('VZGardenTest','VZ_GARDEN: PASS'),@('VZStoryTest','VZ_STORY: PASS'),@('VZHeroTest','VZ_HERO: PASS'),@('VZSmokeTest','VZ_SMOKE: PASS'),@('VZCombatTest','VZ_COMBAT: PASS'),@('VZOrderTest','VZ_ORDERS: PASS'),@('VZMenuTest','VZ_MENU: PASS'))){
    $taskLog=Join-Path $taskRoot ('shared-save-'+$taskTest[0]+'.log')
    ...
    $taskResults | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $taskRoot 'Docs\shared-save-verification.json') -Encoding utf8
